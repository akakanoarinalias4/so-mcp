...
- `so_search`：统一搜索。入参 `query`（必填）/`depth`/`outputType`/`fromDate`/`toDate`/
  ...
  缺省六家搜索同时扇出（tinyfish,tavily,exa,querit,hasdata,firecrawl），多源并行按 URL 去重合并，
  出参 `{provider, providers?, results:[{title,url,content,provider,publishedDate?}], errors?, notes?}`。
- `so_fetch`：统一抓取。入参 `urls`（1..10 条）/`format`/`ttl`/`perUrlTimeoutMs`/`chain`（可选并行名单，上限 8 级）；
  缺省七家抓取按地址轮转分片同时开打，可重试失败换家再并行打一轮封顶成本，
...
- `so_verify`：时效多信源交叉验证。入参 `query`（必填）/`fromDate`/`toDate`/`maxResults`（默认 8）/
  `depth`（默认 standard）/`searchProviders`（默认六家搜索并行）/`fetchChain`（默认七家抓取并行）；
...
- 搜索：缺省六家同时扇出，全部并行逐家结算，失败不阻断其他家；按 URL 去重保留首见归属。
- 抓取：缺省七家按地址轮转分片同时开打，空分片跳过省额度；可重试失败换家再并行打一轮封顶成本。
...
| `TINYFISH_API_KEY` | 按名单 | — | Tinyfish 上游密钥（免费搜索 / 主抓取 / 钱包）。八家任一非空即可运行。 |
| `TAVILY_API_KEY` | 按名单 | — | Tavily 上游密钥（精排搜索 / 难页抓取）。八家任一非空即可运行。 |
| `EXA_API_KEY` | 按名单 | — | Exa 上游密钥（语义搜索 / 提炼抓取）。八家任一非空即可运行。 |
| `QUERIT_API_KEY` | 按名单 | — | Querit 上游密钥（英语补量搜索，免费版仅英文、单次≤10条、单站点）。八家任一非空即可运行。 |
| `HASDATA_API_KEY` | 按名单 | — | HasData 上游密钥（垂直搜索 / 廉价抓取）。八家任一非空即可运行。 |
| `FIRECRAWL_API_KEY` | 按名单 | — | Firecrawl 上游密钥（整站搜索 / 深耕抓取）。八家任一非空即可运行。 |
| `SCRAPEDO_API_KEY` | 按名单 | — | Scrape.do 上游密钥（反爬攻坚抓取）。八家任一非空即可运行。 |
| `SCRAPERAPI_API_KEY` | 按名单 | — | ScraperAPI 上游密钥（结构化兜底抓取）。八家任一非空即可运行。 |
| `SEARCH_PROVIDER` | 否 | `tinyfish` | 单源搜索供应商名（老语义保留）。 |
| `FETCH_PRIMARY` | 否 | `tinyfish` | 单级抓取供应商名（老语义保留）。 |
| `FETCH_FALLBACK` | 否 | `tavily` | 单级回退供应商名（老语义保留）。 |
| `SEARCH_PROVIDERS` | 否 | 六家搜索并行 | 搜索扇出名单（逗号分隔，六家搜索全部并行：tinyfish,tavily,exa,querit,hasdata,firecrawl）。 |
| `FETCH_CHAIN` | 否 | 七家抓取并行 | 抓取并行名单（逗号分隔，上限 8 级，按地址轮转分片同时开打）。 |
| `VERIFY_SEARCH_PROVIDERS` | 否 | 六家搜索并行 | 验证搜索名单（逗号分隔）。 |
| `VERIFY_FETCH_CHAIN` | 否 | 七家抓取并行 | 验证抓取名单（逗号分隔）。 |
...
- 服务端已设 `PROXY_API_KEY` 且八家上游密钥至少配其一，按名单接入，名单为空才停机。
...
上游八家密钥只放 Vercel 服务端环境变量，永不写入客户端配置。
### 3. 客户端配置
...
| `500 代理未配置` | 八家上游密钥均缺配（空名单） | 至少补一上游密钥后重新部署，单键可运行 |
...
3. 密钥不进前端：八家上游密钥只放 Vercel 服务端环境变量，
