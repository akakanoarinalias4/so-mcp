...
// 默认余额端点（deps.tinyfishWalletUrl 优先）。
...
/**
 * 查询 Tavily 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'tavily', balance: null, note: string}>} 占位余额
 */
export const getTavilyBalance = noPublicBalance('tavily');

/**
 * 查询 Exa 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'exa', balance: null, note: string}>} 占位余额
 */
export const getExaBalance = noPublicBalance('exa');

/**
 * 查询 Querit 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'querit', balance: null, note: string}>} 占位余额
 */
export const getQueritBalance = noPublicBalance('querit');

/**
 * 查询 HasData 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'hasdata', balance: null, note: string}>} 占位余额
 */
export const getHasdataBalance = noPublicBalance('hasdata');

/**
 * 查询 Firecrawl 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'firecrawl', balance: null, note: string}>} 占位余额
 */
export const getFirecrawlBalance = noPublicBalance('firecrawl');

/**
 * 查询 Scrape.do 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'scrapedo', balance: null, note: string}>} 占位余额
 */
export const getScrapedoBalance = noPublicBalance('scrapedo');

/**
 * 查询 ScraperAPI 余额（占位：无公开余额接口）。
 * @param {any} _deps 依赖（占位不使用）
 * @returns {Promise<{provider: 'scraperapi', balance: null, note: string}>} 占位余额
 */
export const getScraperapiBalance = noPublicBalance('scraperapi');
