 ...
 *   - SEARCH_PROVIDERS   可选，逗号分隔，默认六家搜索并行（tinyfish,tavily,exa,querit,hasdata,firecrawl）
 *   - FETCH_CHAIN        可选，逗号分隔，默认七家抓取并行，上限 8 级
 *   - VERIFY_SEARCH_PROVIDERS 可选，逗号分隔，默认六家搜索并行
 *   - VERIFY_FETCH_CHAIN      可选，逗号分隔，默认七家抓取并行
 * 上游地址均为固定常量，没有别的开关。
 */
 ...
 * @property {string[]} searchProviders 搜索扇出名单，默认六家搜索并行。
 * @property {string[]} fetchChain      抓取并行名单，默认七家抓取并行，上限 8 级。
 * @property {string[]} verifySearchProviders 验证搜索名单，默认六家搜索并行。
 * @property {string[]} verifyFetchChain 验证抓取名单，默认七家抓取并行。
...
export function resolveConfig(env = process.env) {
  ...
  // 上游密钥按名单必填：八家全空时一次性记入八缺失键，任一非空即可启动。
  ...
  if (!tinyfishApiKey && !tavilyApiKey && !exaApiKey && !queritApiKey && !hasdataApiKey && !firecrawlApiKey && !scrapedoApiKey && !scraperapiApiKey) missing.push(TINYFISH_API_KEY_ENV, TAVILY_API_KEY_ENV, EXA_API_KEY_ENV, QUERIT_API_KEY_ENV, HASDATA_API_KEY_ENV, FIRECRAWL_API_KEY_ENV, SCRAPEDO_API_KEY_ENV, SCRAPERAPI_API_KEY_ENV);
  ...
  // 新增扇出/分级名单：逗号分隔、小写去重；FETCH_CHAIN 上限 8 级（八家并行）。
  const searchProviders = parseProviderList(env[SEARCH_PROVIDERS_ENV], ['tinyfish', 'tavily', 'exa', 'querit', 'hasdata', 'firecrawl']);
  const fetchChain = parseProviderList(env[FETCH_CHAIN_ENV], ['tinyfish', 'tavily', 'exa', 'scrapedo', 'hasdata', 'scraperapi', 'firecrawl'], FETCH_CHAIN_LIMIT);
  const verifySearchProviders = parseProviderList(env[VERIFY_SEARCH_PROVIDERS_ENV], ['tinyfish', 'tavily', 'exa', 'querit', 'hasdata', 'firecrawl']);
  const verifyFetchChain = parseProviderList(env[VERIFY_FETCH_CHAIN_ENV], ['tinyfish', 'tavily', 'exa', 'scrapedo', 'hasdata', 'scraperapi', 'firecrawl']);
...
export function misconfigurationMessage(missing) {
  return (
    ...
    'TINYFISH_API_KEY / TAVILY_API_KEY / EXA_API_KEY / QUERIT_API_KEY / HASDATA_API_KEY / FIRECRAWL_API_KEY / SCRAPEDO_API_KEY / SCRAPERAPI_API_KEY 只注入上游，永不暴露给客户端。'
  );
}
