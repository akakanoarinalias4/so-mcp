 ...
 * @property {string[]} fetchChain      抓取并行名单，默认八家抓取并行，上限 8 级。
 * @property {string[]} verifySearchProviders 验证搜索名单，默认六家搜索并行。
 * @property {string[]} verifyFetchChain 验证抓取名单，默认八家抓取并行。
...
export function resolveConfig(env = process.env) {
  ...
  const fetchChain = parseProviderList(env[FETCH_CHAIN_ENV], ['tinyfish', 'tavily', 'exa', 'scrapedo', 'hasdata', 'scraperapi', 'firecrawl', 'linkup'], FETCH_CHAIN_LIMIT);
  ...
  const verifyFetchChain = parseProviderList(env[VERIFY_FETCH_CHAIN_ENV], ['tinyfish', 'tavily', 'exa', 'scrapedo', 'hasdata', 'scraperapi', 'firecrawl', 'linkup']);
