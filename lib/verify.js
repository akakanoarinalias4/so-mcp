 ...
 * @property {string[]|string} [searchProviders] 搜索扇出名单，默认 tinyfish,tavily
 * @property {string[]|string} [fetchChain] 抓取分级名单，默认 tinyfish,tavily
...
export async function verifyQuery(input, deps) {
  ...
  // 搜索扇出与抓取分级名单：调用方优先，其次配置，最后八家并行缺省。
