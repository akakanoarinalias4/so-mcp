 ...
 * @property {Array<{title: string, url: string, domain: string}>} sources 搜索来源
 * @property {Array<any>} fetched 抓取成功条目
...
export async function verifyQuery(input, deps) {
  ...
  const searchResult = await dispatchTool(
    'so_search',
    { query, fromDate, toDate, maxResults, depth },
    deps,
  );
  const rawResults = Array.isArray(searchResult?.results) ? searchResult.results : [];

  /** @type {Array<{title: string, url: string, domain: string}>} */
  ...
  if (topUrls.length > 0) {
    try {
      const fetchResult = await dispatchTool('so_fetch', { urls: topUrls }, deps);
      ...
      if (fetchResult?.fallbackUsed) notes.push('抓取已使用回退供应商补抓。');
