 ...
 * @property {string[]|string} [searchProviders] 搜索扇出名单，默认 tinyfish,tavily,exa,querit,hasdata,firecrawl
 * @property {string[]|string} [fetchChain] 抓取分级名单，默认 tinyfish,tavily,exa,scrapedo,hasdata,scraperapi,firecrawl
...
export async function verifyQuery(input, deps) {
  ...
  // 搜索扇出与抓取分级名单：调用方优先，其次配置，最后缺省（搜索六家、抓取七家）。
  const searchProviders = normalizeNameList(safeInput.searchProviders)
    ?? readNameList(deps, ['verifySearchProviders', 'searchProviders'])
    ?? ['tinyfish', 'tavily', 'exa', 'querit', 'hasdata', 'firecrawl'];
  const fetchChain = normalizeNameList(safeInput.fetchChain)
    ?? readNameList(deps, ['verifyFetchChain', 'fetchChain'])
    ?? ['tinyfish', 'tavily', 'exa', 'scrapedo', 'hasdata', 'scraperapi', 'firecrawl'];
