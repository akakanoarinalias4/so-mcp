/**
 * lib/tools.js
 * MCP 工具定义与分发：so_search / so_fetch / so_credits / so_verify。
 ...
 * 下游供应商/余额查询统一经 resolveProviderDeps 拍平后调用。
...
/**
 * 四个 MCP 工具的 JSON Schema 定义（供 tools/list 直接返回）。
 * 工具名固定：so_search、so_fetch、so_credits、so_verify。
...
export const TOOL_DEFS = [
  ...
  {
    name: 'so_credits',
    description: '余额查询：并行查询 Linkup 与 Tinyfish 剩余额度。',
    inputSchema: {
      type: 'object',
      properties: {},
...
/**
 * 组装透传给供应商/余额查询的依赖：拍平后的配置 + 原 deps 透传字段（如 fetchImpl）。
 ...
 * 与 errors[] 条目规则一致；无标志的凭证/余额类错误同样尝试回退（回退方可能有可用 Key/额度）。
 ...
 * - so_credits：并行查两家余额，组装 {linkup, tinyfish, checkedAt}，缺 Key 记 skipped。
...
export async function dispatchTool(name, args, deps) {
  ...
  switch (name) {
    ...
    case 'so_credits': {
      // 缺 Key 直接记 skipped，不调用上游；有 Key 则并行查询。
      const checkedAt = new Date().toISOString();
      const hasLinkupKey = Boolean(toolConfig.linkupApiKey);
      const hasTinyfishKey = Boolean(toolConfig.tinyfishApiKey);

      const linkupTask = hasLinkupKey
        ? getLinkupBalance(providerDeps).then(
            (value) => value,
            (error) => ({ status: 'error', error: error instanceof Error ? error.message : String(error) }),
          )
        : Promise.resolve({ status: 'skipped', reason: '缺 LINKUP_API_KEY' });
      const tinyfishTask = hasTinyfishKey
        ? getTinyfishWallet(providerDeps).then(
            (value) => value,
            (error) => ({ status: 'error', error: error instanceof Error ? error.message : String(error) }),
          )
        : Promise.resolve({ status: 'skipped', reason: '缺 TINYFISH_API_KEY' });

      const [linkup, tinyfish] = await Promise.all([linkupTask, tinyfishTask]);
      return { linkup, tinyfish, checkedAt };
    }
