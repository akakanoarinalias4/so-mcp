 ...
 * @property {string[]|string} [chain] 抓取分级名单（可选，按序降级，上限 5 级）
 */
...
export const TOOL_DEFS = [
  {
    name: 'so_search',
    description: '统一搜索：默认单源，可选多源并行扇出（providers）。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'so_fetch',
    description: '统一抓取：默认主备，可选分级链降级（chain，上限 5 级）。',
    inputSchema: {
      type: 'object',
      properties: {
        urls: {
  ...
  switch (name) {
    case 'so_search': {
      ...
      if (!fanout || fanout.length <= 1) {
        ...
        const providerName = (fanout && fanout[0]) || toolConfig.searchProvider || 'linkup';
    ...
    case 'so_fetch': {
      ...
      // 可选 chain 数组：缺省走主备老语义；传入时按序分级，上限 5 级防烧钱。
      ...
      const levels = chain && chain.length > 0 ? chain.slice(0, 5) : [primaryName, fallbackName];
      // 去重保序：同名只保留首次出现。
      /** @type {Array<string>} */
      const ordered = [];
      ...
      /** @type {Array<string>} */
      const usedProviders = [];
      for (let level = 0; level < ordered.length; level += 1) {
        const providerName = ordered[level];
        const fetchFn = getFetchProvider(providerName);
        if (typeof fetchFn !== 'function') {
          throw new Error(`未知抓取供应商: ${providerName}`);
        }
        const targetUrls = level === 0
          ? urls.map((url) => String(url))
          : pendingErrors.filter(isRetryableError).map((item) => item.url);
        // 首级直接打全量；后续级无可重试失败即停。
        if (level > 0 && targetUrls.length === 0) break;
        usedProviders.push(providerName);
        /** @type {{results: Array<any>, errors: Array<any>}} */
        let levelResult;
        try {
          const params = level === 0 ? safeArgs : { ...safeArgs, urls: targetUrls };
          levelResult = normalizeFetchResult(await fetchFn(params, providerDeps));
        } catch (levelError) {
          // 明确不可重试直接上抛；可重试则把目标地址记为失败进入下一级。
          if (!isRetryableThrown(levelError)) throw levelError;
          levelResult = {
            results: [],
            errors: targetUrls.map((url) => ({
              url: String(url),
              error: levelError instanceof Error ? levelError.message : String(levelError),
              retryable: true,
            })),
          };
        }
        // 合并成功，失败进入下一级；不可重试失败直接保留不再补抓。
        mergedResults = [...mergedResults, ...levelResult.results];
        const keptErrors = levelResult.errors.filter((item) => !isRetryableError(item));
        const retryErrors = levelResult.errors.filter(isRetryableError);
        pendingErrors = [...pendingErrors.filter((item) => !isRetryableError(item)), ...keptErrors];
        // 末级或无可重试失败：收尾。
        const isLast = level === ordered.length - 1;
        if (retryErrors.length === 0 || isLast) {
          pendingErrors = [...pendingErrors, ...retryErrors];
          break;
        }
        // 有可重试失败且还有下一级：暂存进入下一级。
        pendingErrors = [...pendingErrors, ...retryErrors];
        // 下一轮只取可重试部分。
        pendingErrors = pendingErrors.filter((item, index, all) => all.findIndex((other) => other.url === item.url) === index);
        // 去重后若全部成功即停由下轮判断。
        const nextRetry = pendingErrors.filter(isRetryableError);
        if (nextRetry.length === 0) break;
        // 将已成功地址从待补集合剔除：仅保留可重试失败。
        pendingErrors = nextRetry;
      }
      return {
