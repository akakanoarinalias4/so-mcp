...
export async function handleCredits(request, input) {
  ...
  // 按名单必填：名单为空说明上游密钥均缺失，直接回 500 代理未配置。
  if (providers.length === 0) return misconfiguredResponse(request, ['LINKUP_API_KEY', 'TINYFISH_API_KEY']);
