...
export async function handleCredits(request, input) {
  ...
  // 名单来自配置对应环境值拍平后重组：有键即接入，空串视为缺失（八家任一非空即可启动）。
  ...
  // 按名单必填：名单为空说明八家上游密钥均缺失，直接回 500 代理未配置。
  if (providers.length === 0) return misconfiguredResponse(request, ['TINYFISH_API_KEY', 'TAVILY_API_KEY', 'EXA_API_KEY', 'QUERIT_API_KEY', 'HASDATA_API_KEY', 'FIRECRAWL_API_KEY', 'SCRAPEDO_API_KEY', 'SCRAPERAPI_API_KEY']);
